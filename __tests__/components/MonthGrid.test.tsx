...
test('shows a photo for a captured day', async () => {
  await render(
    <MonthGrid leadingBlanks={0} days={[{ day: 1, imageUrl: 'https://example.com/x.jpg', state: 'captured' }]} />
  );
...
test('shows a day-number badge over a captured photo', async () => {
  await render(
    <MonthGrid leadingBlanks={0} days={[{ day: 5, imageUrl: 'https://example.com/x.jpg', state: 'captured' }]} />
  );
...
test('shows a frozen indicator on a frozen day', async () => {
  await render(<MonthGrid leadingBlanks={0} days={[{ day: 2, imageUrl: null, state: 'frozen' }]} />);
...
test('renders leading blank cells to align the first day by weekday', async () => {
  await render(<MonthGrid leadingBlanks={3} days={[{ day: 1, imageUrl: null, state: 'future' }]} />);
