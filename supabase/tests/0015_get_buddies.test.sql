...
insert into public.captures (id, user_id, storage_path, captured_at, created_at) values
  ('bbbbbbbb-0000-0000-0000-000000000001', '22222222-2222-2222-2222-222222222222', 'bob-today.jpg', current_date, now() - interval '10 seconds');

-- bob captures a second time today, later than his first capture, to
-- prove get_buddies() resolves to exactly one 'today' row per buddy
-- (the most recent one) rather than duplicating or erroring.
insert into public.captures (id, user_id, storage_path, captured_at, created_at) values
  ('bbbbbbbb-0000-0000-0000-000000000002', '22222222-2222-2222-2222-222222222222', 'bob-today-2.jpg', current_date, now());
...
select is((select count(*) from get_buddies()), 4::bigint, 'buddy count stays 4 even when bob has two captures today, not duplicated to 5');
...
select is((select today_storage_path from get_buddies() where buddy_id = '22222222-2222-2222-2222-222222222222'), 'bob-today-2.jpg', 'the more recent of bob''s two today-captures wins');
...
select react_to_capture('bbbbbbbb-0000-0000-0000-000000000002');
