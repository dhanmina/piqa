-- Fix: get_home_state() top_10 aggregate referenced t.rnk, but t is a column
-- (the jsonb object), not a table alias -> ERROR 42P01 "missing FROM-clause
-- entry for table t". Latent since the rename; only fires when there's a LIVE
-- drop with no PotD and no submissions (a freshly-reset Day 1), which threw the
-- whole RPC so the app showed no drop. Reference the subquery column rnk directly.
