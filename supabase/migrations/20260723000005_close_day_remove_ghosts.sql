-- Auto-remove ghost submissions at reveal (companion to 20260723000004).
--
-- The read-time guard hides ghosts DURING the live window (safe + recoverable: a
-- late upload retry can still rescue the submission). Once close_day runs, the
-- window is over and a ghost can never recover, so it's safe to actually delete
-- it. This keeps the DB clean without the mid-window risk of nuking a submission
-- whose upload was about to land. Ranking is unaffected — ghost votes were
-- already skipped (array_position null) before this cleanup.
...
begin
  ...
  -- Post-close cleanup: a ghost (row whose image upload never landed) can no
  -- longer recover once the drop is closing, so remove it now. It's already
  -- excluded from ids / ranking / gallery above and its votes were ignored
  -- (array_position null -> skipped), so this only clears dead rows (cascading
  -- any blank-pair votes/reactions on them). Live hiding stays the safety net
  -- DURING the window; deletion happens only here, at reveal, when it's truly dead.
