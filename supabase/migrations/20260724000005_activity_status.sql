-- Return each shot's real status (is_potd / in_gallery) so the activity viewer can
-- print the true eyebrow (Photo of the Day / Top 10) and the crown on the frame,
-- instead of nothing. Derived on the client from these two flags — the source of
-- truth is the submission, never the notification kind. Additive to the day_number
-- version; two more fields, ignored by an old client.
...
as $$
  ...
  from (
    select
      ...
      sub.is_potd    as is_potd,
      sub.in_gallery as in_gallery,
