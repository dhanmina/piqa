-- get_gallery: expose the Subject's category on the drop, so the client can tailor
-- the nod picker to the image (see lib/nods.ts nodsFor). Additive field only; the
-- rest of get_gallery (from 20260721000003_fix_gallery_decorate) is unchanged.
...
begin
  ...
  -- the Subject's category, so the client can tailor the nod picker to the image
  select pr.category into cat
  from public.subject_drops pd
  join public.subjects pr on pr.id = pd.prompt_id
  where pd.id = cur_id;
