-- Fix: gallery seed fallback should only show revealed drops, and never
-- fabricate a Photo of the Day crown from live voting data.
--
-- The old seed fallback picked the most recent drop with submissions
-- (regardless of reveal status) and set is_potd = true on the top-voted
-- photo — leaking the crown before voting ended. Now it only picks
-- revealed drops, and the is_potd flag comes from the real close_day
-- data (which is always null in the fallback since no gallery exists).
