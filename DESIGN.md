...
- **Error states:** no hue for error, by palette rule, and no emoji/symbol glyphs in copy either — signaled structurally instead, purely through weight and color: the implicated field's border switches to `textPrimary` (white) via `FilledField`'s `error` prop, paired with a bold `textPrimary` caption from `FieldError` (plain text, no `⚠` or similar prefix). `FieldError` reserves one caption line height (`type.caption.lineHeight`) whether or not a message is shown, so the common case (message fits on one line) never shifts the layout, and text is allowed to actually wrap rather than being cut with `numberOfLines` if it doesn't fit — a rare wrap causing a small one-time shift is accepted as normal (it lands right where the user's already looking, after they typed something or hit submit) rather than paying a permanent double-height cost on every field to prevent it entirely; character-budget estimates for "will this fit on one line" turned out unreliable across real devices/font scales, so copy is kept short by convention, not relied on as a hard guarantee. The fixed cost of the one-line reservation is kept small by grouping each field with its error slot in its own tight (`spacing.xs`) wrapper, rather than letting the outer form's normal rhythm (`spacing.md`) apply between a field and its own caption. Sign-up's username field runs a debounced live availability check (`useUsernameAvailability`) but only speaks up on a problem (bad format, taken, couldn't check) — silent while checking and silent when available, rather than announcing every keystroke's outcome including the boring good one; the enabled "Sign up" button is the positive confirmation. This follows the more common pattern among username-taking apps (Instagram/Twitter-style: silent success, vocal failure) over the alternative of confirming both directions, and better matches the "clarity from contrast and spacing, not decoration" restraint this palette already commits to. Field-error and status copy stays short and specific (e.g. "At least 3 characters", not a generic "Invalid format") because that's better UX on its own terms. Errors attributable to one field (password mismatch, weak password, email already registered) render inline under that field; errors that can't be attributed to one field without leaking information (invalid credentials — never reveal which of email/password was wrong) render in a shared slot at the bottom of the form. Fields clear their own error the moment the user edits them, rather than leaving a stale message. Copy is mapped from Supabase error codes to product language in `lib/authErrors.ts`, not the raw SDK message.
- **Icons:** `expo-symbols`' `SymbolView`, not a generic vector-icon font — resolves to real SF Symbols on iOS and real Material Symbols on Android from one `{ ios, android }` name pair, so nav icons need no rework when iOS ships. Used in `TabBar`; pick platform-native names per icon (e.g. `house`/`house.fill` on iOS, `home`/`home_filled` on Android) rather than porting one platform's icon shape onto the other.
...
| `FieldError` | error message row — plain bold caption (no icon glyph), reserves one line-height so appearing/disappearing never shifts layout in the common (one-line) case |
